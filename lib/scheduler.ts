...
const JOBS = [
  { scope: "events", label: "이벤트", cron: EVENTS_CRON, staleHours: EVENTS_STALE_HOURS, read: readEvents, collect: collectEvents },
  { scope: "deals", label: "핫딜", cron: DEALS_CRON, staleHours: DEALS_STALE_HOURS, read: readDeals, collect: collectDeals },
] as const;
  ...
  for (const job of JOBS) {
    cron.schedule(
      ...
      () => {
        ...
        void runExclusive(job.scope, job.collect).catch((error) => {
